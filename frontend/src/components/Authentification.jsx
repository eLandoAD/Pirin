...
function Input({ label, name, type = "text", value, onChange, hint }) {
  return (
    <div className="flex flex-col gap-1">
      <label htmlFor={name} className="text-[13px] font-medium text-slate-800">{label}</label>
      <input
  ...
  return (
    <>
      ...
      <p className="m-0 mb-6 text-[13px] text-slate-500">
        ...
        <button onClick={() => onSwitch("signup")} className="bg-transparent border-none text-green-dark cursor-pointer text-[13px] p-0 font-semibold">Sign Up</button>
      </p>
...
function SignUpView({ onSwitch }) {
  ...
  return (
    <>
      <h2 className="m-0 mb-1 text-[20px] font-bold text-slate-900">Create account</h2>
      <p className="m-0 mb-6 text-[13px] text-slate-500">
        You already have an account?{" "}
        <button onClick={() => onSwitch("login")} className="bg-transparent border-none text-green-dark cursor-pointer text-[13px] p-0 font-semibold">Log In</button>
      </p>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
...
// Vista reset password — aperta dal link nell'email
function ResetView({ resetToken, onClose }) {
  ...
  if (done) {
    return (
      <div className="text-center py-4">
        <div className="text-[40px] mb-4">✅</div>
        <h2 className="m-0 mb-2 text-[18px] font-semibold text-slate-900">Password updated!</h2>
        <p className="text-[13px] text-slate-500 mb-6">
  ...
  return (
    <>
      <h2 className="m-0 mb-1 text-[20px] font-bold text-slate-900">Set new password</h2>
      <p className="m-0 mb-6 text-[13px] text-slate-500">Enter your new password below.</p>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <Input label="New password" name="password" type="password" value={form.password} onChange={update} hint="At least 8 characters" />
        <Input label="Confirm password" name="confirm" type="password" value={form.confirm} onChange={update} />
        {error && <p className="rounded-md bg-red-50 px-3 py-2 text-[13px] text-red-600 m-0">{error}</p>}
        <button type="submit" disabled={loading}
          className={`mt-1 w-full rounded-md bg-slate-900 border-none p-2.5 text-[13px] font-semibold text-white cursor-pointer ${loading ? 'opacity-70' : 'opacity-100'}`}>
...
export default function AuthModal({ initialView = "login", onClose, onLoginSuccess, resetToken }) {
  ...
  return (
    <div onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div onClick={(e) => e.stopPropagation()}
        ...
        <button onClick={onClose} className="absolute right-4 top-4 bg-transparent border-none text-[16px] text-slate-400 cursor-pointer">✕</button>
        ...
        {view === "reset"  && <ResetView  resetToken={resetToken} onClose={onClose} />}
      </div>
    </div>
  );
}
