          ...
          <>
            ...
            {files.length > 0 && (
              <div className="mb-4 flex flex-col gap-1.5">
                {files.map((f, i) => {
                  const status = fileStatus[f.name];
                  ...
                  const barColor =
                    ...
                    phase === "encrypting" ? "bg-slate-400" : "bg-slate-200";
                        ...
                        <div className="ml-3 flex items-center gap-2">
                          {phaseLabel && (
                            <span className={`text-[11px] font-medium ${
                              phase === "done"       ? "text-green" :
                              phase === "encrypting" ? "text-slate-400" : "text-green"
                            }`}>
                              {phaseLabel}
                            </span>
                          )}
                          {status?.loaded !== undefined && phase === "uploading" && (
                          ...
                          <button
                            ...
                            disabled={uploading}
                            className="bg-transparent border-none cursor-pointer text-slate-500 p-0 disabled:cursor-not-allowed disabled:opacity-50"
                          >
                            <X size={14} />
                          </button>
                        </div>
                        ...
                        <div className="h-1 w-full bg-slate-200">
                          <div
                            className={`h-1 transition-all duration-300 ${barColor} ${
                              phase === "encrypting" ? "animate-pulse w-full" : ""
                            }`}
                            style={phase !== "encrypting" ? { width: `${percentage}%` } : undefined}
                          />
                        </div>
                      )}
                    </div>
